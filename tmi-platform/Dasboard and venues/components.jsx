// ─── TMI Shared Primitives ────────────────────────────────────────────────────
// BerntoutGlobal · The Musician's Index
// Import these into each dashboard. No props are optional unless noted.

import { useEffect } from 'react';
import { C, FONTS, FONT_IMPORT, KEYFRAMES } from './tmiTokens';
  ...
  useEffect(() => {
    ...
    style.textContent = `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ${KEYFRAMES}
      .tmi-mq     { animation: mq     1.9s ease-in-out infinite; }
      .tmi-mqB    { animation: mqB    1.9s ease-in-out infinite; }
      .tmi-blink  { animation: blink  1.2s ease-in-out infinite; }
      .tmi-sway   { animation: sway   2s   ease-in-out infinite; transform-origin: bottom center; }
      .tmi-float  { animation: floatUp 1.5s ease-out   forwards; }
      .tmi-ticker { animation: ticker  14s  linear     infinite; }
      .tmi-flicker{ animation: flicker 4s   ease-in-out infinite; }
      .tmi-pulse  { animation: pulse   2s   ease-in-out infinite; }
      @keyframes tmiSlideInRight { from { transform: translateX(100%); } to { transform: translateX(0); } }
      @keyframes tmiSlideInLeft  { from { transform: translateX(-100%); } to { transform: translateX(0); } }
...
// ── AudienceSVG ───────────────────────────────────────────────────────────────
export function AudienceSVG({ count = 21 }) {
    ...
    <div>
      ...
      {data[0]?.[labelKey] !== undefined && (
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 2 }}>
          {data.map((d, i) => (
            <span key={i} style={{ fontSize: 7, color: C.dim, flex: 1, textAlign: 'center' }}>
              {d[labelKey]}
            </span>
          ))}
        </div>
          ...
          <div key={i} style={{ flex: 1, height: `${(totals[i] / max) * 100}%`, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
            {keys.map((k, j) => (
              <div
                key={k}
                style={{
                  height:    `${((d[k] || 0) / totals[i]) * 100}%`,
                  background: colors[j] || C.red,
                  marginTop:  j > 0 ? 1 : 0,
                  borderRadius: j === keys.length - 1 ? '2px 2px 0 0' : 0,
                  minHeight:  2,
                }}
              />
            ))}
...
// ── ActionCanister (Rotating Menu for In-Place Actions) ───────────────────────
export function ActionCanister({ actions = [], activeActionId, onSelect }) {
  return (
    <div style={{
      display: 'flex',
      ...
      {actions.map(action => {
        ...
        return (
          <button
            ...
            style={{
              background: isActive ? 'rgba(230,48,0,.25)' : 'transparent',
              border: `1px solid ${isActive ? C.gold : 'transparent'}`,
              color: isActive ? C.gold : C.amber,
              padding: '10px 14px',
              borderRadius: 6,
              cursor: 'pointer',
              fontFamily: FONTS.body,
...
// ── WidgetDrawer (Slide-in Panel to prevent Page Redirects) ───────────────────
export function WidgetDrawer({ isOpen, onClose, title, children, position = 'right' }) {
  ...
  return (
    <div style={{
      ...
      animation: `tmiSlideIn${position === 'right' ? 'Right' : 'Left'} 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards`,
      boxShadow: position === 'right' ? '-20px 0 50px rgba(0,0,0,0.8)' : '20px 0 50px rgba(0,0,0,0.8)',
      backdropFilter: 'blur(20px)'
    }}>
      <div style={{ padding: '16px', borderBottom: `1px solid rgba(255,255,255,0.1)`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontFamily: FONTS.display, color: C.gold, fontSize: 14, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{title}</span>
        <button onClick={onClose} style={{ background: 'transparent', border: 'none', color: C.red, cursor: 'pointer', fontSize: 24, lineHeight: 1 }}>×</button>
      </div>
      <div className="tmi-scroll" style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        {children}
      </div>
    </div>
  );
