...
export default function OmniPresenceEngine() {
  ...
  const [tileMode, setTileMode] = useState('speaking');
        ...
        {activeTab === 'videotiles' && (
          <div className="fade-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px' }}>
              {['Tiana (TG)', 'Julius', 'Redbeard', 'SByeeGil'].map((name, i) => (
                <div key={name} style={{ background: C.panel, borderRadius: '12px', overflow: 'hidden', border: i === 0 ? `2px solid ${C.gd}` : `2px solid ${C.bd}`, boxShadow: i === 0 ? `0 0 15px ${C.gd}44` : 'none' }}>
                  ...
                  <div style={{ padding: '8px 12px', background: C.card }}>
                    ...
                    <div style={{ fontSize: '10px', color: C.mt }}>{['Performer', 'Hype Master', 'Co-Host', 'Fan'][i]}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="fade-in">
            ...
            <div style={{ background: C.card, padding: '24px', borderRadius: '12px', border: `1px solid ${C.bd}` }}>
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: C.cy, fontWeight: 800, marginBottom: '8px' }}>Voice Volume <span>{voiceVol}%</span></label>
              ...
              <div>
                <label style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: C.gd, fontWeight: 800, marginBottom: '8px' }}>Beat / Music {voiceVol > 70 && ' (DUCKED)'} <span>{duckedBeat}%</span></label>
                <input type="range" min="0" max="100" value={duckedBeat} disabled style={{ width: '100%', opacity: 0.5, cursor: 'not-allowed' }} />
              </div>
            </div>
          </div>
        )}
