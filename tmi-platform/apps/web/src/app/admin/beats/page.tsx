...
export default function AdminBeatsPage() {
  ...
  function act(id: string, action: "approve" | "remove") {
    setBeats(prev => prev.map(b => b.id === id ? { ...b, status: action === "approve" ? "LIVE" : "REMOVED" } : b));
  }
              ...
              <tr key={b.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td style={{ padding: "14px 12px" }}>
                  <div style={{ display: "flex", gap: 6 }}>
                    {b.status !== "LIVE" && <button onClick={() => act(b.id, "approve")} style={{ padding: "4px 10px", fontSize: 8, fontWeight: 800, color: "#00FF88", border: "1px solid rgba(0,255,136,0.3)", borderRadius: 4, background: "transparent", cursor: "pointer" }}>APPROVE</button>}
                    {b.status !== "REMOVED" && <button onClick={() => act(b.id, "remove")} style={{ padding: "4px 10px", fontSize: 8, fontWeight: 800, color: "#FF2DAA", border: "1px solid rgba(255,45,170,0.3)", borderRadius: 4, background: "transparent", cursor: "pointer" }}>REMOVE</button>}
                  </div>
                </td>
              </tr>
            ))}
