...
type Flag = { id: string; type: string; reporter: string; target: string; reason: string; severity: string; status: string };
    ...
    <main className="min-h-screen bg-[#0a0a0f] text-white px-6 py-10">
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", marginTop: 4 }}>
          {flags.filter(f => f.status !== "Resolved").length} open · {flags.filter(f => f.severity === "High" && f.status !== "Resolved").length} high severity
        </p>
      </div>
        ...
        {flags.map((f) => (
          <div key={f.id} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${f.status !== "Resolved" ? "rgba(255,68,68,0.15)" : "rgba(255,255,255,0.07)"}`, borderRadius: 12, padding: "14px 18px", display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              {f.status !== "Resolved" && (
                <button onClick={() => resolve(f.id)} style={{ fontSize: 11, fontWeight: 700, color: "#00FFAA", background: "rgba(0,255,170,0.1)", border: "1px solid rgba(0,255,170,0.2)", borderRadius: 6, padding: "4px 10px", cursor: "pointer" }}>Resolve</button>
              )}
            </div>
          </div>
        ))}
