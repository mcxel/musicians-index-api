...
type Payout = { id: string; recipient: string; type: string; amount: number; status: string; method: string; period: string };
          ...
          <tbody>
            {payouts.map(p => (
              <tr key={p.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td style={{ padding: "14px 12px" }}>{p.status === "PENDING" && <button onClick={() => payNow(p.id)} style={{ padding: "4px 10px", fontSize: 8, fontWeight: 800, color: "#00FF88", border: "1px solid rgba(0,255,136,0.3)", borderRadius: 4, background: "transparent", cursor: "pointer" }}>PAY NOW</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
