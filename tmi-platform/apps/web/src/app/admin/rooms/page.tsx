...
type RoomStatus = 'LIVE' | 'SCHEDULED' | 'IDLE' | 'ENDED';
type Room = { id: string; name: string; type: string; status: RoomStatus; occupancy: number; capacity: number; host: string | null; region: string; startedAt: string };
...
const TYPE_ICONS: Record<string, string> = { THEATER: '🎭', CLUB: '🎵', VIP: '⭐', ARENA: '🏟️', BAR: '🍺' };
const STATUS_STYLES: Record<string, string> = {
  LIVE: 'bg-red-500/20 text-red-400 border-red-500/30', SCHEDULED: 'bg-blue-500/20 text-blue-400 border-blue-500/30', IDLE: 'bg-gray-500/20 text-gray-400 border-gray-500/30', ENDED: 'bg-gray-500/10 text-gray-500 border-gray-500/20',
};
...
export default function AdminRoomsPage() {
  ...
  function endRoom(id: string) {
    setRooms(prev => prev.map(r => r.id === id ? { ...r, status: 'ENDED', occupancy: 0 } : r));
  }

  function monitorRoom(id: string) {
    setMsg(`Monitoring: ${rooms.find(r => r.id === id)?.name}`);
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] text-white px-6 py-10 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/admin/big-ace/sites" className="px-4 py-2 bg-[#ff6b35] hover:bg-[#ff6b35]/80 text-white text-sm font-semibold rounded-xl transition-colors">
          + Create Room
        </Link>
      </div>
              ...
              <div key={room.id} className="bg-red-500/5 border border-red-500/20 rounded-2xl p-4">
                ...
                <div className="mb-2">
                  ...
                  <div className="w-full h-1.5 bg-white/10 rounded-full">
                    <div className="h-1.5 bg-red-400 rounded-full" style={{ width: `${Math.round((room.occupancy / room.capacity) * 100)}%` }} />
                  </div>
                </div>
                <p className="text-xs text-gray-500 mb-3">Host: {room.host} · {room.startedAt}</p>
                <div className="flex gap-2">
                  <button onClick={() => monitorRoom(room.id)} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-xs text-gray-300 rounded-lg transition-colors">Monitor</button>
                  <button onClick={() => endRoom(room.id)} className="flex-1 py-1.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-xs text-red-400 rounded-lg transition-colors">End Room</button>
                </div>
              </div>
            ))}
          </div>
            ...
            <tbody className="divide-y divide-white/5">
              {rooms.map((room) => (
                <tr key={room.id} className="hover:bg-white/3 transition-colors">
                  ...
                  <td className="px-4 py-4 text-xs text-gray-300">{room.occupancy}/{room.capacity}</td>
                  <td className="px-4 py-4">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${STATUS_STYLES[room.status]}`}>{room.status}</span>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex gap-1">
                      <button onClick={() => monitorRoom(room.id)} className="px-2 py-1 bg-white/5 hover:bg-white/10 border border-white/10 text-xs text-gray-300 rounded transition-colors">View</button>
                      {room.status === 'LIVE' && <button onClick={() => endRoom(room.id)} className="px-2 py-1 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-xs text-red-400 rounded transition-colors">End</button>}
                    </div>
                  </td>
                </tr>
              ))}
