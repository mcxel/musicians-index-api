...
type Lead = { company: string; type: string; status: string; value: number; rep: string };
...
const NEXT_STATUS: Record<string, string> = { OUTREACH: "REVIEW", REVIEW: "NEGOTIATING", NEGOTIATING: "SIGNED", SIGNED: "SIGNED" };
...
export default function AdminSalesPage() {
  ...
  function updateLead(company: string) {
    const lead = leads.find(l => l.company === company);
    if (!lead) return;
    const next = NEXT_STATUS[lead.status] ?? lead.status;
    setLeads(prev => prev.map(l => l.company === company ? { ...l, status: next } : l));
    setMsg(`${company}: ${lead.status} → ${next}`);
            ...
            {leads.map((l, i) => (
              <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td style={{ padding: "14px 12px" }}>
                  {l.status !== "SIGNED" && (
                    <button onClick={() => updateLead(l.company)} style={{ padding: "4px 10px", fontSize: 8, fontWeight: 800, color: "#00FFFF", border: "1px solid rgba(0,255,255,0.3)", borderRadius: 4, background: "transparent", cursor: "pointer" }}>UPDATE</button>
                  )}
                </td>
              </tr>
            ))}
