...
type Item = { id: string; name: string; category: string; price: string; currency: string; stock: number; sold: number; status: ItemStatus; rarity: string };
...
const RARITY_COLORS: Record<string, string> = { COMMON: 'text-gray-400', RARE: 'text-blue-400', EPIC: 'text-purple-400', LEGENDARY: 'text-yellow-400' };
const STATUS_STYLES: Record<string, string> = { ACTIVE: 'bg-green-500/20 text-green-400 border-green-500/30', DRAFT: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30', ARCHIVED: 'bg-gray-500/20 text-gray-400 border-gray-500/30' };
...
export default function AdminStorePage() {
  ...
  function editItem(id: string) {
    setMsg(`Edit: ${items.find(i => i.id === id)?.name}`);
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] text-white px-6 py-10 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/avatar/shop" className="px-4 py-2 bg-[#ff6b35] hover:bg-[#ff6b35]/80 text-white text-sm font-semibold rounded-xl transition-colors">
          + Add Item
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {[
          ...
          { label: 'Revenue', value: '$12,847', color: 'text-yellow-400' },
        ].map((s) => (
        ...
        {['ALL', 'AVATAR', 'EFFECT', 'PASS'].map((cat) => (
          <button
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors ${
              filter === cat ? 'bg-[#ff6b35] border-[#ff6b35] text-white' : 'bg-white/5 border-white/10 text-gray-400 hover:text-white'
            }`}
          >
            {cat}
          </button>
        ...
        {displayed.map((item) => (
          <div key={item.id} className="bg-white/5 border border-white/10 rounded-2xl p-5">
            <div className="flex items-start justify-between mb-3">
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${STATUS_STYLES[item.status]}`}>{item.status}</span>
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={() => editItem(item.id)} className="flex-1 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-xs text-gray-300 rounded-lg transition-colors">Edit</button>
              {item.status !== 'ARCHIVED' && <button onClick={() => archiveItem(item.id)} className="flex-1 py-1.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-xs text-red-400 rounded-lg transition-colors">Archive</button>}
            </div>
          </div>
        ))}
      </div>
