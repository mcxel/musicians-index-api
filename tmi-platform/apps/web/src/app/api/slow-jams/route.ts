...
import {
  getSlowJamsPoolStatus,
  submitToSlowJamsPool,
  SJ_SUBMIT_COIN_RESERVE,
} from "@/lib/radio/SlowJamsRotationPool";
...
export async function GET() {
  ...
  const pool = getSlowJamsPoolStatus(schedule.weekKey);
  ...
  return NextResponse.json({
    ...
    atmosphere: {
      ...
      skinId: atmosphere.skinId,
      label: atmosphere.label,
      assetAvailable: atmosphere.assetAvailable,
      venueIndex: atmosphere.venueIndex,
    },
    ...
    fees: {
      submitCoinReserve: SJ_SUBMIT_COIN_RESERVE,
    ...
    walletSample: null as { coins: number; xp: number } | null,
  });
}
...
export async function POST(req: NextRequest) {
  ...
  const result = submitToSlowJamsPool({
    submitterId: userId,
    artistName,
    title,
    audioUrl: url,
  ...
  return NextResponse.json({
    ...
    message:
      ...
      "Queued for Sunday Slow Jams — points charged only if your track plays",
  });
}
