...
/**
 * DB-backed atomic inventory check using EventInventory.
 * Returns "ok" | "sold_out" | "invalid_limit".
 * On DB unavailability, returns "ok" so ticketEngine in-memory path can still mint
 * (CERT-ENV vs PRODUCTION: absence of migrate/table is environment-scoped, not a global "DB missing").
 */
