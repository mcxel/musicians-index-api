...
import { TMIHeadline, TMIConfetti, TMIPill, TMILiveBadge } from '@/components/shared/TMIGeoBlock';
...
export default async function ArticleChannelPage({ params }: { params: Promise<{ channel: string }> }) {
  const { channel } = await params;
  const cfg = CHANNELS[channel as ChannelKey];
  if (!cfg) notFound();
    ...
    <div style={{ minHeight: '100vh', background: '#050510', color: '#f8f7f1', fontFamily: "'Bebas Neue','Impact',sans-serif", position: 'relative', overflowX: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(16px,3vw,32px)', position: 'relative', zIndex: 2 }}>
          ...
          {CHANNEL_KEYS.map(k => {
            ...
            return (
              ...
              >
                <div style={{
                  padding: '8px 14px',
                  background: k === channel ? `${c.color}22` : 'transparent',
                  borderBottom: k === channel ? `3px solid ${c.color}` : '3px solid transparent',
                  fontFamily: "'Inter',sans-serif", fontSize: 9, fontWeight: 900,
                  letterSpacing: '0.14em', textTransform: 'uppercase',
                  color: k === channel ? c.color : 'rgba(255,255,255,0.4)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr 1fr', gap: 14, marginBottom: 28 }}>
              ...
              <Link key={article.id} href={`/articles/${article.slug}`} style={{ textDecoration: 'none' }}>
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
                    ...
                    <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 9, fontWeight: 900, color: accent, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                      READ FULL STORY →
                    </div>
                  </div>
                </div>
          ...
          {/* Column 2 — Live feeds / article mix */}
            ...
            {liveFeeds.slice(0, 3).map(feed => (
              <Link key={feed.id} href={feed.href} style={{ textDecoration: 'none' }}>
                ...
                >
                  ...
                  <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 8, fontWeight: 900, color: feed.accentColor, letterSpacing: '0.1em', textTransform: 'uppercase', flexShrink: 0 }}>
                    JOIN →
                  </div>
                </div>
              </Link>
          ...
          {/* Column 3 — Fan poll + sponsor + article */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>

            {/* Fan Poll block */}
            <div style={{ background: 'rgba(255,45,170,0.08)', border: '2px solid #FF2DAA44', padding: 16 }}>
              <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 8, fontWeight: 900, color: '#FF2DAA', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 10 }}>
                🗳️ FAN POLL
              </div>
              <div style={{ fontFamily: "'Bebas Neue','Impact',sans-serif", fontSize: 16, color: '#fff', marginBottom: 12 }}>
            ...
            {articles.slice(5, 8).map(article => (
              <Link key={article.id} href={`/articles/${article.slug}`} style={{ textDecoration: 'none' }}>
                ...
                >
                  ...
                  <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 9, color: 'rgba(255,255,255,0.5)', marginTop: 4 }}>
                    {article.snippet?.slice(0, 80) ?? ''}
                  </div>
                </div>
              </Link>
            ))}
            ...
            <Link key={article.id} href={`/articles/${article.slug}`} style={{ textDecoration: 'none' }}>
              <div
                ...
                style={{
                  ...
                  border: `1px solid ${accent}22`,
                  padding: 14,
                  clipPath: CARD_SHAPES[idx % CARD_SHAPES.length] !== 'jagg'
                    ? undefined
                    : 'polygon(0% 3%, 97% 0%, 100% 97%, 3% 100%)',
                  transition: 'transform 0.2s ease',
                  animation: `tmiCardIn ${0.2 + idx * 0.05}s ease`,
                }}
              >
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 7, fontWeight: 900, color: accent, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 6 }}>
                  {cfg.icon} {cfg.label}
                </div>
                <div style={{ fontFamily: "'Bebas Neue','Impact',sans-serif", fontSize: 16, color: '#fff', lineHeight: 1.1 }}>
                  {article.title || `Trending in ${cfg.label}`}
                </div>
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 9, color: 'rgba(255,255,255,0.5)', marginTop: 6 }}>
                  {article.snippet?.slice(0, 70) ?? ''}
                </div>
              </div>
            </Link>
          ))}
        ...
        <div style={{ background: accent, padding: '18px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 9, color: 'rgba(255,255,255,0.85)', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              FRONT-PAGE PLACEMENT · SPONSOR WALLS · BILLBOARD TILES
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <Link href="/advertisers" style={{ textDecoration: 'none' }}>
              <div style={{ background: '#fff', color: accent, padding: '10px 24px', fontFamily: "'Bebas Neue','Impact',sans-serif", fontSize: 16, boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
                ADVERTISER HUB →
              </div>
            </Link>
            <Link href="/sponsors" style={{ textDecoration: 'none' }}>
              <div style={{ background: 'rgba(0,0,0,0.25)', color: '#fff', padding: '10px 24px', fontFamily: "'Bebas Neue','Impact',sans-serif", fontSize: 16, border: '2px solid rgba(255,255,255,0.4)' }}>
                SPONSOR ARTIST
              </div>
            </Link>
          </div>
