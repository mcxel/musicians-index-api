...
type Offer = { id: string; venue: string; city: string; date: string; budget: string; status: OfferStatus; receivedAt: string; genre: string; message: string | null };
...
const STATUS_STYLES: Record<string, string> = {
  PENDING: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30', ACCEPTED: 'bg-green-500/20 text-green-400 border-green-500/30', DECLINED: 'bg-red-500/20 text-red-400 border-red-500/30', EXPIRED: 'bg-gray-500/20 text-gray-400 border-gray-500/30',
};
            ...
            {pending.map((offer) => (
              <div key={offer.id} className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${STATUS_STYLES[offer.status]}`}>{offer.status}</span>
                </div>
                {offer.message && (
                ...
                <div className="flex gap-2">
                  <button onClick={() => respond(offer.id, 'ACCEPTED')} className="flex-1 py-2 bg-green-500/20 hover:bg-green-500/30 border border-green-500/30 text-green-400 text-sm font-semibold rounded-lg transition-colors">Accept</button>
                  <button onClick={() => respond(offer.id, 'DECLINED')} className="flex-1 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 text-sm font-semibold rounded-lg transition-colors">Decline</button>
                </div>
              </div>
            ))}
          </div>
