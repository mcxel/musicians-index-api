...
export default function FanSlugError({
  error,
  reset,
}: {
    ...
    <main className="min-h-screen bg-zinc-950 p-6 text-white">
      <div className="mx-auto grid max-w-3xl gap-4">
        <header className="rounded-xl border border-rose-400/35 bg-rose-950/20 p-5">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-rose-300">Fan Route Error</p>
          <h1 className="mt-1 text-2xl font-black uppercase tracking-tight">Recover Fan Segment</h1>
          <p className="mt-2 text-xs text-zinc-300">A recoverable error occurred while rendering the fan slug route.</p>
          {error?.message ? (
