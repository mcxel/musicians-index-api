...
const TICKETS = [
  { id: "t1", event: "Monday Cypher #42",     date: "May 13, 2026", status: "ACTIVE" },
  { id: "t2", event: "Wavetek Live",           date: "May 20, 2026", status: "ACTIVE" },
  { id: "t3", event: "Battle Arena S1 Final",  date: "May 27, 2026", status: "UPCOMING" },
];
...
export default function FanTicketsPage() {
  ...
  function viewTicket(id: string) {
    router.push("/fan/" + id + "/tickets/nft");
  }
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#050510", color: "#fff", padding: "40px 24px" }}>
      <div style={{ maxWidth: 920, margin: "0 auto" }}>
        ...
        {msg && <div style={{ margin: "16px 0", padding: "10px 14px", background: "rgba(255,45,170,0.08)", border: "1px solid rgba(255,45,170,0.2)", borderRadius: 8, fontSize: 12, color: "#FF2DAA" }}>{msg}</div>}

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12, marginTop: 32 }}>
          {TICKETS.map((ticket) => (
            <div key={ticket.id} style={{ border: "1px solid rgba(255,45,170,0.25)", borderRadius: 10, padding: "16px", background: "rgba(255,45,170,0.04)" }}>
              ...
              <button onClick={() => viewTicket(ticket.id)} style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid rgba(255,45,170,0.4)", background: "rgba(255,45,170,0.1)", color: "#FF2DAA", cursor: "pointer", fontWeight: 700, fontSize: 10 }}>
                View Ticket
              </button>
            </div>
          ))}
        ...
        <div style={{ marginTop: 32, border: "1px solid rgba(255,255,255,0.14)", borderRadius: 12, padding: "16px" }}>
          <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 4 }}>Points Balance: 420</div>
