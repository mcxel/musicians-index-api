...
const VIEWER_COUNTS = [412, 287, 538, 201, 344, 189];

export default function CountryEventsPage() {
  const params = useParams();
  const router = useRouter();
  const rawCountry = params?.country;
  const country = typeof rawCountry === "string" ? rawCountry : Array.isArray(rawCountry) ? rawCountry[0] : "";
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-950 via-black to-gray-950">
      <div className="border-b border-gray-800 bg-black/40 backdrop-blur-sm sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <h1 className="text-3xl font-bold text-cyan-300 mb-2">🎪 Events in {countryCode}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="p-4 rounded-lg bg-gray-900/50 border border-gray-800 hover:border-cyan-500/50 transition-all">
              ...
              <p className="text-sm text-gray-400 mb-3">Live now · {VIEWER_COUNTS[i - 1]} viewers</p>
              <button
                onClick={() => router.push("/arena?country=" + country + "&event=" + i)}
