...
export default function LiveHostsPage() {
  ...
  useEffect(() => { fetchHost(); }, [venueSlug]);

  async function fetchHost() {
    setLoading(true);
    try {
      const res = await fetch(`/api/live/host?venue=${venueSlug}`);
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#050510", color: "#fff", paddingBottom: 80 }}>
      <section style={{ padding: "56px 24px 32px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        <h1 style={{ fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 900, marginBottom: 8 }}>Host Runtime</h1>
        <p style={{ color: "#999", fontSize: 14 }}>Host mic control, stage scripts, cue queue, emergency override.</p>
      </section>
      ...
      <section style={{ maxWidth: 820, margin: "0 auto", padding: "40px 24px" }}>
        ...
        {!host && !loading && (
          <form onSubmit={registerHost} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid #333", borderRadius: 12, padding: "24px 28px", marginBottom: 24 }}>
            <h3 style={{ fontWeight: 800, fontSize: 15, marginBottom: 18 }}>Register Host</h3>
            ...
            <button type="submit"
              style={{ background: "#FF8C00", color: "#050510", border: "none", borderRadius: 8, padding: "9px 24px", fontWeight: 800, fontSize: 13, cursor: "pointer" }}>
              Register Host
            </button>
          </form>
        )}
          ...
          <>
            <div style={{ background: "rgba(255,140,0,0.07)", border: "1px solid #FF8C0033", borderRadius: 12, padding: "22px 26px", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
                <span style={{ fontSize: 13, color: host.micActive ? "#00FF88" : "#555" }}>Mic: {host.micActive ? "HOT 🎤" : "OFF"}</span>
                <span style={{ fontSize: 13, color: host.cameraActive ? "#00BFFF" : "#555" }}>Camera: {host.cameraActive ? "LIVE" : "OFF"}</span>
              </div>
              {host.currentScript && (
                <div style={{ background: "#0d0d1f", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#FFD700", marginBottom: 12 }}>
                  Script: {host.currentScript}
                </div>
              )}
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid #333", borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}>
              <h4 style={{ fontWeight: 800, fontSize: 13, marginBottom: 12 }}>Queue Cue</h4>
              <div style={{ display: "flex", gap: 10 }}>
                <input value={cueScript} onChange={(e) => setCueScript(e.target.value)} placeholder="Cue script text..."
                  style={{ flex: 1, background: "#0d0d1f", border: "1px solid #333", borderRadius: 7, color: "#fff", padding: "8px 12px", fontSize: 13 }} />
                <button onClick={() => { hostAction("queue-cue", { cueType: "banter", script: cueScript }); setCueScript(""); }}
                  disabled={!cueScript}
                  style={{ background: "#AA2DFF", color: "#fff", border: "none", borderRadius: 7, padding: "8px 16px", fontWeight: 800, cursor: "pointer", fontSize: 13 }}>
                  Queue
                </button>
              </div>
              <p style={{ fontSize: 11, color: "#666", marginTop: 6 }}>Cues in queue: {host.cueQueue.length}</p>
            </div>
            ...
            <form onSubmit={makeAnnouncement} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid #333", borderRadius: 12, padding: "20px 24px" }}>
              <h4 style={{ fontWeight: 800, fontSize: 13, marginBottom: 12 }}>Make Announcement</h4>
              <div style={{ display: "flex", gap: 10 }}>
              ...
              {host.announcementLog.length > 0 && (
                <div style={{ marginTop: 12 }}>
                  {host.announcementLog.slice(-3).map((log, i) => (
                    <p key={i} style={{ fontSize: 11, color: "#666", margin: "2px 0" }}>{log}</p>
                  ))}
                </div>
              )}
            </form>
