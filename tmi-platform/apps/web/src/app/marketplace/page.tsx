...
const FILTER_MAP: Record<string, string> = { ALL: "", BEATS: "BEAT", PACKS: "PACK", COLLABS: "COLLAB" };

export default function MarketplacePage() {
  const router = useRouter();
  const [activeFilter, setActiveFilter] = useState("ALL");

  const displayed = activeFilter === "ALL" ? LISTINGS : LISTINGS.filter(l => l.type === FILTER_MAP[activeFilter]);
    ...
    <main style={{ minHeight: "100vh", background: "#050510", color: "#fff", paddingBottom: 80 }}>
      <section style={{ textAlign: "center", padding: "56px 24px 40px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          {["ALL", "BEATS", "PACKS", "COLLABS"].map((f) => (
            <button key={f} onClick={() => setActiveFilter(f)} style={{ padding: "7px 16px", fontSize: 9, fontWeight: 700, color: activeFilter === f ? "#050510" : "rgba(255,255,255,0.5)", background: activeFilter === f ? "#00FFFF" : "rgba(255,255,255,0.05)", border: `1px solid ${activeFilter === f ? "#00FFFF" : "rgba(255,255,255,0.1)"}`, borderRadius: 20, cursor: "pointer" }}>{f}</button>
          ))}
        </div>
      </section>
        ...
        {displayed.map(item => (
          <div key={item.id} style={{ position: "relative", background: "rgba(255,255,255,0.02)", border: `1px solid ${item.color}20`, borderRadius: 12, padding: "20px 18px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => router.push(`/api/stripe/checkout?product=marketplace_${item.id}&mode=payment`)} style={{ fontSize: 8, fontWeight: 800, color: "#050510", background: item.color, borderRadius: 6, padding: "8px 16px", border: "none", cursor: "pointer" }}>BUY NOW</button>
            </div>
          </div>
        ))}
      </section>
