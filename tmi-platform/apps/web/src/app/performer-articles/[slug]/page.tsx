...
export async function generateStaticParams() {
  return getLocalArticlesByCategory("performer").map(a => ({ slug: a.slug }));
}

export async function generateMetadata({ params }: Props) {
  const { slug } = await params;
  const article = getLocalArticleBySlug(slug);
  if (!article || article.category !== "performer") return { title: "Not Found | TMI" };
  return {
    title: `${article.title} | TMI Performer Articles`,
    description: article.subtitle,
  };
}

export default async function PerformerArticlePage({ params }: Props) {
  const { slug } = await params;
  const article = getLocalArticleBySlug(slug);
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#050510", color: "#fff", paddingBottom: 80 }}>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "24px 24px 0" }}>
        <Link href="/performer-articles" style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.15em", color: "rgba(255,255,255,0.3)", textDecoration: "none" }}>
          ← PERFORMER ARTICLES
        </Link>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 24px 40px", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 32 }}>
        <div style={{ display: "flex", gap: 14 }}>
          <Link href="/performer-articles" style={{ padding: "10px 22px", fontSize: 9, fontWeight: 800, letterSpacing: "0.12em", color: "#FF2DAA", border: "1px solid rgba(255,45,170,0.3)", borderRadius: 8, textDecoration: "none" }}>
            MORE PERFORMER ARTICLES
          </Link>
          <Link href="/hub/performer" style={{ padding: "10px 22px", fontSize: 9, fontWeight: 800, letterSpacing: "0.12em", color: "#050510", background: "linear-gradient(135deg,#FF2DAA,#AA0088)", borderRadius: 8, textDecoration: "none" }}>
            PERFORMER HUB →
          </Link>
        </div>
      </div>
