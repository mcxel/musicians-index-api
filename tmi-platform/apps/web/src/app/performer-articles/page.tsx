...
export const metadata: Metadata = {
  title: "Performer Articles | TMI",
  description: "In-depth articles for performers, DJs, and stage artists on the TMI platform.",
};

export default function PerformerArticlesPage() {
  const articles = getLocalArticlesByCategory("performer");
    ...
    <main style={{ minHeight: "100vh", background: "#050510", color: "#fff", paddingBottom: 80 }}>
      <section style={{ textAlign: "center", padding: "64px 24px 40px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", maxWidth: 460, margin: "0 auto 24px" }}>
          Strategy, playbooks, and insights for performers building audiences and booking revenue on TMI.
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <Link href="/artist-articles" style={{ padding: "7px 18px", border: "1px solid rgba(0,255,255,0.3)", borderRadius: 20, color: "#00FFFF", textDecoration: "none", fontSize: 10, fontWeight: 700, letterSpacing: "0.12em" }}>Artist Articles</Link>
        </div>
      </section>

      <section style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px" }}>
        {articles.length === 0 ? (
          <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", padding: 60 }}>No performer articles yet. Check back soon.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {articles.map(article => (
              <Link key={article.slug} href={`/performer-articles/${article.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
                <article style={{ display: "flex", gap: 20, alignItems: "flex-start", background: "rgba(255,255,255,0.02)", border: `1px solid ${article.heroColor}18`, borderRadius: 14, padding: "24px 28px", cursor: "pointer" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.2em", color: article.heroColor, marginBottom: 6 }}>
                      PERFORMER ARTICLE · {article.publishedAt}
                    </div>
