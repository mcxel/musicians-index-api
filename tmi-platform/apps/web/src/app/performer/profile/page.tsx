...
const STATS = [
  ...
  { label: "XP",            value: "48,200", color: ACCENT    },
  { label: "Battle Record", value: "22–3",   color: "#00FFFF" },
  { label: "Monthly Fans",  value: "12.4K",  color: "#FF2DAA" },
  { label: "Earnings",      value: "$3,240", color: "#00FF88" },
  { label: "Streams",       value: "94K",    color: "#FFD700" },
];

const RECENT_BATTLES = [
  { opp: "Zion Freq",  result: "WIN",  xp: "+320 XP", date: "May 28" },
  { opp: "Wave Tek",   result: "WIN",  xp: "+280 XP", date: "May 22" },
  { opp: "DJ Phantom", result: "LOSS", xp: "−120 XP", date: "May 15" },
  { opp: "Big Ace",    result: "WIN",  xp: "+410 XP", date: "May 8"  },
];

const UPCOMING = [
  { title: "Cypher Arena Friday",  date: "Jun 6 · 9PM ET",  href: "/shows/friday-cypher" },
  { title: "Monthly Idol Round 3", date: "Jun 8 · 8PM ET",  href: "/shows/monthly-idol"  },
  { title: "Battle Championship",  date: "Jun 14 · 7PM ET", href: "/battles/championship" },
];

const GENRES = ["EDM", "Hip-Hop", "Trap", "R&B"];

export default function PerformerProfilePage() {
  const [editing, setEditing] = useState(false);
  const [displayName, setDisplayName] = useState("Nova Cipher");
  const [bio, setBio] = useState("Multi-genre performer and battle artist. Crown holder Season 1. Available for bookings and collabs.");
  const [genres, setGenres] = useState("EDM, Hip-Hop");
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1000, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "flex-start", padding: "28px 28px", background: `linear-gradient(135deg, ${ACCENT}10, rgba(5,5,16,0.95))`, border: `1px solid ${ACCENT}28`, borderRadius: 20, marginBottom: 24, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ fontSize: 9, letterSpacing: "0.25em", color: ACCENT, fontWeight: 800, marginBottom: 4 }}>PERFORMER · BATTLE ARTIST</div>
            <h1 style={{ margin: "0 0 6px", fontSize: "clamp(22px,4vw,34px)", fontWeight: 900 }}>{displayName}</h1>
            <p style={{ margin: "0 0 10px", fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.5, maxWidth: 480 }}>{bio}</p>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {GENRES.map((g) => <span key={g} style={{ padding: "3px 10px", borderRadius: 20, fontSize: 9, fontWeight: 700, background: `${ACCENT}18`, border: `1px solid ${ACCENT}35`, color: ACCENT }}>{g}</span>)}
            </div>
          </div>
        ...
        {editing && (
          <div style={{ padding: "24px 28px", background: "rgba(255,255,255,0.02)", border: `1px solid ${ACCENT}22`, borderRadius: 16, marginBottom: 24 }}>
            ...
            <form action="/api/profile/update" method="POST" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                <button type="submit" style={{ padding: "11px 28px", borderRadius: 9, background: ACCENT, color: "#fff", border: "none", fontSize: 12, fontWeight: 900, cursor: "pointer", letterSpacing: "0.08em" }}>
                  SAVE CHANGES
                </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ padding: "20px 24px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14 }}>
            ...
            {RECENT_BATTLES.map((b) => (
              <div key={b.opp} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700 }}>vs {b.opp}</div>
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,0.35)", marginTop: 1 }}>{b.date}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 10, fontWeight: 900, color: b.result === "WIN" ? "#00FF88" : "#FF4444" }}>{b.result}</div>
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,0.35)" }}>{b.xp}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: "20px 24px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14 }}>
            ...
            {UPCOMING.map((u) => (
              <Link key={u.href} href={u.href} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", textDecoration: "none", color: "#fff" }}>
                <div>
