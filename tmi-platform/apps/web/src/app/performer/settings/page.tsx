...
const CFG = { label: 'Performer', accent: '#AA2DFF', hub: '/hub/performer' };
const S = { bg: '#07071a', border: '#AA2DFF33', text: '#fff', muted: '#888', card: '#0f0f2a' };

export default function PerformerSettingsPage() {
  const [notifs, setNotifs] = useState({ battles: true, messages: true, revenue: true, news: false });
  const [visibility, setVisibility] = useState('public');

  return (
    <div style={{ minHeight: '100vh', background: S.bg, color: S.text, fontFamily: 'sans-serif', padding: '2rem' }}>
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        ...
        <Section title="Account" accent={CFG.accent}>
          ...
          <SaveBtn accent={CFG.accent} label="Update Account" />
        </Section>
        ...
        <Section title="Privacy" accent={CFG.accent}>
          <label style={{ display: 'block', fontSize: 13, color: S.muted, marginBottom: 8 }}>Profile Visibility</label>
        ...
        <Section title="Subscription" accent={CFG.accent}>
          <p style={{ color: S.muted, fontSize: 14, marginBottom: '1rem' }}>Current plan: <strong style={{ color: CFG.accent }}>Free</strong></p>
        ...
        <Section title="Danger Zone" accent="#ff4444">
          <p style={{ color: S.muted, fontSize: 14, marginBottom: '1rem' }}>Permanently deactivate your account and remove your data.</p>
...
function SaveBtn({ accent, label }: { accent: string; label: string }) {
  return (
