...
function roleToSegment(role: string | null): 'fans' | 'performers' {
  if (!role) return 'fans';
  const r = role.toLowerCase();
  if (r === 'performer' || r === 'performers' || r === 'artist' || r === 'band') return 'performers';
  return 'fans';
}

function PricingPageInner() {
  const searchParams = useSearchParams();
  const initialSegment = roleToSegment(searchParams.get('role'));
  const [segment, setSegment] = useState<'fans' | 'performers'>(initialSegment);
    ...
    <>
      ...
      >
        <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%' }}>
          ...
          {/* Toggle — Fans | Performers (primary surface) */}
                ...
                <button
                  ...
                  style={{
                    ...
                    background:
                      segment === seg
                        ? seg === 'fans'
                          ? 'linear-gradient(135deg,#00FFFF,#00DDFF)'
