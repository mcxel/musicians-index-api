...
const C = { bg: '#07071a', panel: 'rgba(12,20,50,.9)', border: '#1a1a3a', accent: '#38bdf8', gold: '#FFD700', fuchsia: '#FF2DAA', purple: '#AA2DFF', green: '#00FF88', text: '#fff', dim: '#666' };
...
export default function PromoterProfilePage() {
  ...
  const ACTIONS = [
    ...
    { label: 'Analytics',     icon: '📊', href: '/dashboard/promoter', color: C.green   },
    { label: 'Sponsor Deals', icon: '🤝', href: '/sponsor/campaigns',  color: C.accent  },
    { label: 'Advertising',   icon: '📢', href: '/advertising',        color: C.fuchsia },
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: 'linear-gradient(135deg, #050815, #001428)', borderBottom: `1px solid ${C.border}`, padding: '36px 28px 24px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 30% 50%, rgba(56,189,248,0.08) 0%, transparent 70%)' }} />
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 9, letterSpacing: '0.35em', color: C.accent, fontWeight: 800, marginBottom: 4 }}>PROMOTER PROFILE</div>
            <h1 style={{ margin: 0, fontSize: 26, fontWeight: 900 }}>{name}</h1>
            <span style={{ fontSize: 11, color: C.accent, fontWeight: 700 }}>{session?.tier ?? 'FREE'} · PROMOTER ACCOUNT</span>
          </div>
        </div>
      </div>
      <div style={{ padding: '24px', maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 24 }}>
          {[{ label: 'Events Promoted', value: '0', icon: '🎪', color: C.accent }, { label: 'Artists Booked', value: '0', icon: '🎤', color: C.fuchsia }, { label: 'Tickets Sold', value: '0', icon: '🎟', color: C.gold }, { label: 'Revenue', value: '$0', icon: '💰', color: C.green }].map(s => (
        ...
        <div style={{ fontSize: 9, letterSpacing: '0.3em', color: C.dim, fontWeight: 700, marginBottom: 12 }}>PROMOTER TOOLS</div>
