...
export default function PropsPage() {
  ...
  function buyProp(id: string, label: string, price: number) {
    router.push(`/api/stripe/checkout?priceId=price_prop_${id}&mode=payment&name=${encodeURIComponent(label)}&coins=${price}`);
  }
  ...
  return (
    <main data-testid="props-page" style={{ minHeight: "100vh", background: "#020617", color: "#e2e8f0", padding: 20 }}>
      ...
      {msg && <div style={{ marginBottom: 12, padding: "8px 12px", background: "rgba(165,243,252,0.08)", border: "1px solid rgba(165,243,252,0.2)", borderRadius: 8, fontSize: 12, color: "#a5f3fc" }}>{msg}</div>}
        ...
        {PROPS.map(({ id, label, price, type }) => (
          <div key={id} data-testid={`prop-${id}`} style={{ border: "1px solid rgba(148,163,184,0.2)", borderRadius: 12, padding: 16, background: "rgba(15,23,42,0.8)" }}>
            ...
            <div style={{ marginTop: 12 }}>
              <button data-testid={`buy-prop-${id}`} type="button" onClick={() => buyProp(id, label, price)} style={{ border: "1px solid rgba(165,243,252,0.3)", borderRadius: 6, background: "rgba(165,243,252,0.08)", color: "#a5f3fc", fontSize: 11, padding: "5px 12px", cursor: "pointer" }}>{price} coins</button>
            </div>
          </div>
        ))}
      </div>
