...
const DEFAULT_CAPS: BusinessPartnerCapability[] = ["ADVERTISER"];
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/session", { credentials: "include", cache: "no-store" })
      ...
      .then((d: { authenticated?: boolean }) => {
        if (active && d?.authenticated) router.replace("/hub/advertiser");
      })
      .catch(() => {});
    return () => { active = false; };
  }, [router]);
    ...
    <OnboardingShell
      ...
      form={(
        <BusinessPartnerSignupFields
          accentColor="#AA2DFF"
          heading="Launch Campaign Control"
          subheading="One business account — advertise, sponsor events, artists, or prize pools from a single dashboard."
          form={form}
          onChange={setForm}
