...
export default function SponsorAnalyticsPage() {
  ...
  const snapshot = getAnalyticsSnapshot(tier, 'sponsor');
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#07071a', color: '#e2e8f0', minHeight: '100vh', padding: '32px 24px 64px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontSize: 10, color: '#FF9200', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 6 }}>Sponsor Analytics</div>
            <h1 style={{ fontSize: 26, fontWeight: 900, margin: 0 }}>Sponsorship Performance</h1>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            <Link href="/hub/sponsor" style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textDecoration: 'none', marginRight: 8 }}>← Hub</Link>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 800, marginBottom: 10 }}>Active Engines</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {capabilities.enabledEngines.map((eng) => {
              const meta = ENGINE_META[eng];
              return <div key={eng} style={{ fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 6, border: `1px solid ${meta.color}44`, background: `${meta.color}11`, color: meta.color }}>◉ {meta.label}</div>;
            })}
          </div>
        </div>
        ...
        {upgradePrompt && (
          <div style={{ background: 'rgba(255,146,0,0.06)', border: '1px solid rgba(255,146,0,0.2)', borderRadius: 12, padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)' }}>{upgradePrompt}</div>
            <Link href="/subscriptions" style={{ fontSize: 11, fontWeight: 800, color: '#07071a', background: '#FF9200', borderRadius: 7, padding: '9px 18px', textDecoration: 'none', whiteSpace: 'nowrap' }}>Upgrade</Link>
          </div>
        )}
      </div>
    </div>
