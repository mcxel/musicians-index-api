...
export default function TicketDetailPage() {
    ...
    return (
      <main style={{ minHeight: "100vh", background: "#050510", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <Link href="/tickets" style={{ color: "#00FFFF", textDecoration: "none" }}>← Browse Events</Link>
        </div>
      </main>
    );
  }
  ...
  function buyTicket() {
    router.push(`/api/stripe/checkout?priceId=price_ticket_${eventId}&mode=payment&qty=${qty}&name=${encodeURIComponent(event.title)}`);
  }
    ...
    <main style={{ minHeight: "100vh", background: "#050510", color: "#fff", paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "32px 24px", display: "grid", gridTemplateColumns: "1fr 300px", gap: 36, alignItems: "start" }}>
        <div>
          ...
          <h1 style={{ fontSize: "clamp(1.4rem,4vw,2.2rem)", fontWeight: 900, lineHeight: 1.2, marginBottom: 8 }}>{event.title}</h1>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, fontSize: 11 }}>
              ...
              <span style={{ color: remaining < 20 ? "#FF2DAA" : "rgba(255,255,255,0.4)" }}>{remaining} remaining</span>
            </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${event.color}20`, borderRadius: 16, padding: 24, position: "sticky", top: 80 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[1, 2, 4].map(q => (
                <button key={q} onClick={() => setQty(q)} style={{ flex: 1, padding: "8px 0", fontSize: 13, fontWeight: 800, color: qty === q ? "#050510" : "rgba(255,255,255,0.5)", background: qty === q ? event.color : "rgba(255,255,255,0.04)", border: `1px solid ${qty === q ? event.color : "rgba(255,255,255,0.1)"}`, borderRadius: 8, cursor: "pointer" }}>
                  {q}
                </button>
              ))}
            </div>
          ...
          <button onClick={buyTicket} style={{ display: "block", width: "100%", textAlign: "center", padding: "13px 0", fontSize: 11, fontWeight: 800, letterSpacing: "0.15em", color: "#050510", background: `linear-gradient(135deg,${event.color},${event.color}99)`, borderRadius: 10, border: "none", cursor: "pointer", marginBottom: 10 }}>
            BUY {qty > 1 ? `${qty}x ` : ""}TICKET — ${(event.price * qty).toFixed(2)}
          </button>
