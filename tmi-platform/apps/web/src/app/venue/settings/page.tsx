...
const CFG = { label: 'Venue', accent: '#22c55e', hub: '/hub/venue' };
    ...
    <div style={{ minHeight: '100vh', background: '#07071a', color: '#fff', fontFamily: 'sans-serif', padding: '2rem' }}>
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        ...
        <Section title="Account" accent={CFG.accent}>
          ...
          <SaveBtn accent={CFG.accent} dark />
        </Section>
        ...
        <Section title="Privacy" accent={CFG.accent}>
          <label style={{ display: 'block', fontSize: 13, color: '#888', marginBottom: 8 }}>Venue Visibility</label>
        ...
        <Section title="Subscription" accent={CFG.accent}>
          ...
          <Link href="/subscribe" style={{ background: CFG.accent, color: '#07071a', borderRadius: 8, padding: '0.6rem 1.5rem', textDecoration: 'none', fontSize: 14, fontWeight: 700 }}>
            Upgrade Plan
          </Link>
        ...
        <Section title="Danger Zone" accent="#ff4444">
          <p style={{ color: '#888', fontSize: 14, marginBottom: '1rem' }}>Permanently deactivate your venue account and remove your data.</p>
...
function SaveBtn({ accent, dark }: { accent: string; dark?: boolean }) {
  return (
    <button type="button" style={{ background: accent, color: dark ? '#07071a' : '#fff', border: 'none', borderRadius: 8, padding: '0.6rem 1.5rem', fontSize: 14, cursor: 'pointer', fontWeight: 700, marginTop: 8 }}>
      Update Account
    </button>
  );
}
