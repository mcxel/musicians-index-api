...
function ViewportBody({ source }: { source: MediaSourceDefinition }) {
  return (
...
function MediaRenderer({ source }: { source: MediaSourceDefinition }) {
  ...
  if (source.kind === "security") {
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1.25fr 0.95fr", gap: 8, minHeight: 118 }}>
        <MonitorViewport source={source} label="Sentinel cam" meta="Alert rotation · camera grid" />
  ...
  return <MonitorViewport source={source} label={source.label} meta={source.detail} />;
}

function MonitorViewport({ source, label, meta }: { source: MediaSourceDefinition; label: string; meta: string }) {
  return (
    <div style={{ minHeight: 118, borderRadius: 8, border: `1px solid ${source.accent}33`, overflow: "hidden", background: "#04060b", display: "flex", flexDirection: "column" }}>
      <div style={{ position: "relative", minHeight: 98 }}>
        <video
          ...
          loop
          playsInline
          style={{ width: "100%", height: "100%", objectFit: "cover", filter: source.kind === "security" ? "saturate(0) contrast(1.15)" : source.kind === "battle" ? "saturate(1.2) contrast(1.05)" : "saturate(0.95)" }}
        />
        ...
        <div style={{ position: "absolute", top: 8, left: 8, display: "flex", alignItems: "center", gap: 6, padding: "3px 7px", borderRadius: 999, background: "rgba(0,0,0,0.62)", border: `1px solid ${source.status === "LIVE" ? "rgba(0,255,136,0.35)" : "rgba(255,215,0,0.35)"}` }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: source.status === "LIVE" ? "#00FF88" : "#FFD700", boxShadow: `0 0 8px ${source.status === "LIVE" ? "#00FF88" : "#FFD700"}` }} />
          <span style={{ color: "#fff", fontSize: 8, fontWeight: 900, letterSpacing: "0.14em", textTransform: "uppercase" }}>{source.status}</span>
        </div>
      ...
      <div style={{ display: "flex", alignItems: "end", justifyContent: "space-between", gap: 10, padding: "8px 10px", borderTop: `1px solid ${source.accent}2f`, background: "rgba(2,4,8,0.86)" }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ color: "rgba(255,255,255,0.72)", fontSize: 9, lineHeight: 1.35, maxWidth: 220, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{meta}</div>
        </div>
