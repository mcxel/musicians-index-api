...
export default function GenreArenaView({ slug }: { slug: string }) {
  const cfg = GENRE_CONFIG[slug] ?? GENRE_CONFIG['hip-hop']!;
  const [slots, setSlots] = useState<BattleSlot[]>(buildQueue(SEED_PERFORMERS));
  ...
  return (
    <main style={{ minHeight: '100vh', background: cfg.bg, color: '#fff', fontFamily: "'Inter', sans-serif" }}>
      <style>{`@keyframes arenaPulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
      ...
      {/* Live Audience Scene — 3D crowd renders behind the stage */}
      <div style={{ position: 'relative', height: 180, overflow: 'hidden', borderBottom: `1px solid ${cfg.accent}22` }}>
        <AudienceScene
          view="performer"
          venue={1}
          watcherCount={audience}
          accentColor={cfg.accent}
          screenLabel={`${cfg.emoji} ${cfg.name.toUpperCase()} ARENA`}
          screenSubLabel="LIVE NOW · FREE TO WATCH"
        />
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 60, background: `linear-gradient(to top, ${cfg.bg}, transparent)` }} />
      </div>
          ...
          <div style={{ background: `linear-gradient(135deg, ${cfg.accent}14, rgba(5,5,16,0.95))`, border: `1px solid ${cfg.accent}33`, borderRadius: 16, padding: '20px 24px', marginBottom: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 12, alignItems: 'center' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <button onClick={() => setVotes(v => ({ ...v, [liveSlot.id]: 1 }))} style={{ marginTop: 10, padding: '7px 20px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: 900, background: votes[liveSlot.id] === 1 ? cfg.accent : 'rgba(255,255,255,0.08)', color: votes[liveSlot.id] === 1 ? '#050510' : '#fff', letterSpacing: '0.08em' }}>
                  🔥 VOTE A
                </button>
              </div>
              <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <button onClick={() => setVotes(v => ({ ...v, [liveSlot.id]: 2 }))} style={{ marginTop: 10, padding: '7px 20px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: 900, background: votes[liveSlot.id] === 2 ? '#FF2DAA' : 'rgba(255,255,255,0.08)', color: votes[liveSlot.id] === 2 ? '#050510' : '#fff', letterSpacing: '0.08em' }}>
                  🎤 VOTE B
                </button>
              </div>
            </div>
            ...
            {slots.filter(s => s.status !== 'live').map((slot, i) => (
              <div key={slot.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 18px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)' }}>{slot.format.replace('-', ' ')} · {slot.prize}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: i === 0 ? '#FFD700' : 'rgba(255,255,255,0.3)' }}>{fmtCountdown(slot.startIn)}</div>
                  <div style={{ fontSize: 8, color: 'rgba(255,255,255,0.2)', marginTop: 1 }}>{slot.status === 'upcoming' ? 'UP NEXT' : 'OPEN'}</div>
                </div>
              </div>
            ))}
            <div style={{ padding: '14px 18px' }}>
              <Link href={`/battles/challenge/new?genre=${slug}&arena=true`} style={{ display: 'block', textAlign: 'center', padding: '10px', borderRadius: 8, background: `${cfg.accent}14`, border: `1px solid ${cfg.accent}35`, color: cfg.accent, fontSize: 10, fontWeight: 900, textDecoration: 'none', letterSpacing: '0.08em' }}>
                + JOIN THE QUEUE
              </Link>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ flex: 1, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <form onSubmit={sendChat} style={{ display: 'flex', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
              <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Say something..." maxLength={120} style={{ flex: 1, background: 'transparent', border: 'none', padding: '10px 14px', color: '#fff', fontSize: 12, outline: 'none' }} />
              <button type="submit" style={{ padding: '10px 14px', background: 'transparent', border: 'none', color: cfg.accent, fontSize: 14, cursor: 'pointer', fontWeight: 900 }}>↑</button>
            </form>
          </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {[
                ...
                { label: '🔥 HYPE',   href: '/arena',                                        color: '#FF2DAA' },
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {Object.entries(GENRE_CONFIG).filter(([k]) => k !== slug).slice(0, 8).map(([s, g]) => (
                <Link key={s} href={`/arena/${s}`} style={{ fontSize: 9, fontWeight: 700, color: g.accent, border: `1px solid ${g.accent}33`, borderRadius: 4, padding: '3px 8px', textDecoration: 'none', background: `${g.accent}08` }}>
                  {g.emoji} {g.name}
                </Link>
              ))}
            </div>
