...
export function AvatarCoHostCharacter({ identity, presence, size = "md", showName = true, showTier = false, className = "" }: AvatarCoHostCharacterProps) {
  const sizeMap = { sm: "w-12 h-12", md: "w-16 h-16", lg: "w-24 h-24", xl: "w-32 h-32" };

  return (
    <div
      className={`flex flex-col items-center gap-1 ${className}`}
      data-avatar-role="cohost"
      data-zone="interview-chair"
    >
...
export default AvatarCoHostCharacter;
