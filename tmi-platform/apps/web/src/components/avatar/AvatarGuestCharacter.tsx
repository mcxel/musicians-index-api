...
export function AvatarGuestCharacter({ identity, presence, size = "md", showName = true, showTier = false, className = "" }: AvatarGuestCharacterProps) {
  const sizeMap = { sm: "w-12 h-12", md: "w-16 h-16", lg: "w-24 h-24", xl: "w-32 h-32" };

  return (
    <div
      className={`flex flex-col items-center gap-1 ${className}`}
      data-avatar-role="guest"
...
export default AvatarGuestCharacter;
