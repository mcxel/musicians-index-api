...
export interface AvatarHostCharacterProps {
...
export function AvatarHostCharacter({ identity, presence, size = "md", showName = true, showTier = false, className = "" }: AvatarHostCharacterProps) {
  const sizeMap = { sm: "w-12 h-12", md: "w-16 h-16", lg: "w-24 h-24", xl: "w-32 h-32" };

  return (
    <div
      className={`flex flex-col items-center gap-1 ${className}`}
      data-avatar-role="host"
      data-zone="host-podium"
    >
...
export default AvatarHostCharacter;
