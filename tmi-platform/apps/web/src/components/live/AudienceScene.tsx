...
const AudienceSceneJumbotronLayer = dynamic(
  () =>
    import("@/components/jumbotron/AudienceSceneJumbotronLayer").then(
      (m) => m.AudienceSceneJumbotronLayer,
    ),
  { ssr: false },
);
...
export {
  ...
  FAN_ROWS,
  PERF_ROWS,
  computeAudienceSeatPins,
  type AudienceSeatPin,
} from "@/lib/audience/audienceSeatGeometry";
import {
  AUDIENCE_SCENE_W,
  AUDIENCE_SCENE_H,
  FAN_TOTAL_SEATS,
...
interface AudienceSceneProps {
  ...
  /**
   * Physical Jumbotron (AutomatedJumbotronDirector geometry) — center-hung /
   * end-zone / disco orb / wall LED. Mounted in world-space over the canvas.
   */
  jumbotron?: {
    roomId: string;
...
export default function AudienceScene({
  ...
  return (
    <div style={{ position: "relative" }} data-audience-bobblehead-overlay={bobbleOverlays.length > 0 ? "true" : "false"}>
      ...
      />
      {jumbotron?.roomId ? (
        <AudienceSceneJumbotronLayer
          roomId={jumbotron.roomId}
          venue={venue}
          eventType={jumbotron.eventType}
          venueId={jumbotron.venueId}
          lookUpActive={jumbotron.lookUpActive === true}
        />
      ) : null}
