...
function LobbyCell({ room, colorIndex, onJoin, onPrewarm }: { room: LobbyRoom; colorIndex: number; onJoin: (room: LobbyRoom) => void; onPrewarm: (room: LobbyRoom) => void }) {
  ...
  const isPrivate = room.visibility === 'private' || !joinable;
  ...
  return (
    <motion.div
      ...
      whileHover={joinable ? { scale: 1.04, zIndex: 10 } : undefined}
      onClick={joinable ? () => onJoin(room) : undefined}
      onMouseEnter={joinable ? () => onPrewarm(room) : undefined}
      onTouchStart={joinable ? () => onPrewarm(room) : undefined}
      style={{
        position: 'relative',
        borderRadius: 12,
        aspectRatio: '4/3',
    ...
    >
      ...
      {room.competitiveLifecycleState === 'preparing' && (
        <div style={{
          position: 'absolute', top: 30, right: 24,
          fontSize: 8, fontWeight: 900, letterSpacing: '0.12em',
          color: '#FFD700', background: 'rgba(5,5,16,0.75)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {isPrivate && <span style={{ fontSize: 9, color: '#fff' }}>🔒 Preview only</span>}
          {room.competitiveLifecycleState === 'waiting_for_contender' && <span style={{ fontSize: 9, color: '#FFD700' }}>⏳ Waiting contender</span>}
          {(room.competitiveLifecycleState === 'opponent_joined' || room.competitiveLifecycleState === 'vs_animation' || room.competitiveLifecycleState === 'countdown') && (
            <span style={{ fontSize: 9, color: '#FFD700' }}>⚔️ Battle starting</span>
          )}
...
export default function LiveLobbyWallGrid({ rooms, title, accentColor = '#00FFFF', typeLabel = 'LIVE' }: LiveLobbyWallGridProps) {
  ...
  const visibleRooms = rooms.filter((r) => r.status !== 'ended');
  const joinableRooms = visibleRooms.filter((r) => r.isJoinable !== false);
  ...
  const joinRandom = useCallback(() => {
    if (joinableRooms.length === 0) return;
    const pick = joinableRooms[Math.floor(Math.random() * joinableRooms.length)];
    if (pick) joinRoom(pick);
  }, [joinableRooms, joinRoom]);
    ...
    <div style={{ minHeight: '100vh', background: '#050510', color: '#fff', paddingBottom: 80 }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'linear-gradient(180deg, rgba(5,5,16,0.95) 0%, rgba(5,5,16,0.8) 100%)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 11, color: '#00FF88' }}>
            ...
            {' '}{visibleRooms.filter((room) => room.status === 'live').length} LIVE
          </span>
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={joinRandom}
            disabled={joinableRooms.length === 0}
            style={{
              padding: '10px 22px',
              ...
              fontWeight: 900,
              fontSize: 12,
              cursor: joinableRooms.length > 0 ? 'pointer' : 'not-allowed',
              letterSpacing: '0.1em',
            }}
          >
            🎲 RANDOM ROOM
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 20px' }}>
        <div style={{ marginBottom: 24, width: '100%' }}>
          <AdRailSlot slotId="lobby-wall-featured" hasSponsor={false} />
        </div>
