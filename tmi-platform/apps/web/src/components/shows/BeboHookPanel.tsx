...
/** Shape returned by BeboHookEngine.getState() */
...
export function BeboHookPanel({ beboState, onHook, onReturn, activeContestantId, recentBooCount = 0, recentYayCount = 0 }: BeboHookPanelProps) {
  ...
  const sustainedProgress = Math.min(beboState.sustainedBooTicks / beboState.minSustainedTicks, 1);
  ...
  return (
    ...
    >
      ...
      {/* Sustained boo progress bar (only visible when in warning stage) */}
      {isWarned && (
